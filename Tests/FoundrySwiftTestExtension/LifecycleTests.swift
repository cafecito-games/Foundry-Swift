...
final class LifecycleTests {
    ...
    public func testFreeResource() {
        func createImageAndGetId () -> Int64 {
            ...
            XCTAssertTrue (Foundry.isInstanceIdValid(id: id), "Image was supposed to be alive")
            return id
        }
        let id = createImageAndGetId()
        releasePendingObjects()
        XCTAssertFalse (Foundry.isInstanceIdValid(id: id), "Expected image to be disposed")
    }
    ...
    public func testFreeObjects() {
        func createTimerAndGetId () -> Int64 {
            let img = UndoRedo()
            let id = Int64(bitPattern: UInt64(img.getInstanceId()))
            XCTAssertTrue (Foundry.isInstanceIdValid(id: id), "Timer was supposed to be alive")
            img.free()
            return id
        }
        let id = createTimerAndGetId()
        XCTAssertFalse (Foundry.isInstanceIdValid(id: id), "Expected timer to be disposed")
    }
    ...
    public func testEngineReturnedFileAccessKeepsSingleNativeReference() {
        let testPath = "user://foundryswift-engine-returned-refcount.txt"
