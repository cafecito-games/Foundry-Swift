...
//
//  Tests for Codable round-trips of Variant and Foundry builtin types.
//
...
final class CodableTests {
    ...
    public func testGStringCodable() {
        let original = GString("Hello, Foundry!")
        let data = try! encoder.encode(original)
    ...
    public func testPackedStringArrayCodable() {
        let original = PackedStringArray(["hello", "world", "foundry"])
        let data = try! encoder.encode(original)
    ...
    public func testVariantDictionaryNestedInVariantArray() {
        ...
        XCTAssertEqual(String(Variant(takingOver: decodedDict["name"]!)!), "Foundry")
