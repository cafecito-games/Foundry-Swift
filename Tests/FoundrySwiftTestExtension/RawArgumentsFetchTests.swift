...
// The tests use `node.call(method:...)` which goes through Foundry's dynamic dispatch mechanism,
...
// MARK: - Custom FoundryBuiltinConvertible types for testing

/// A custom type backed by Int
struct CustomIntWrapper: FoundryBuiltinConvertible {
    let value: Int

    public func toFoundryBuiltin() -> Int { value }

    public static func fromFoundryBuiltinOrThrow(_ value: Int) throws(VariantConversionError) -> Self {
        CustomIntWrapper(value: value)
    }
}

/// A custom type backed by String
struct CustomStringWrapper: FoundryBuiltinConvertible {
    let text: String

    public func toFoundryBuiltin() -> String { text }

    public static func fromFoundryBuiltinOrThrow(_ value: String) throws(VariantConversionError) -> Self {
        CustomStringWrapper(text: value)
    }
}

/// A custom type backed by Vector2
struct CustomVector2Wrapper: FoundryBuiltinConvertible {
    let vec: Vector2

    public func toFoundryBuiltin() -> Vector2 { vec }

    public static func fromFoundryBuiltinOrThrow(_ value: Vector2) throws(VariantConversionError) -> Self {
...
class RawArgumentsTestNode: RefCounted {
    ...
    // MARK: - Primitive types via _FoundryBridgeableBuiltin
    ...
    // MARK: - Generated builtin types via _FoundryBridgeableBuiltin
    ...
    // MARK: - Array types via _FoundryBridgeableBuiltin
    ...
    // MARK: - TypedArray and TypedDictionary via _FoundryBridgeableBuiltin
    ...
    // MARK: - Swift Array and Dictionary via _FoundryBridgeableBuiltin
    ...
    // MARK: - FoundryBuiltinConvertible custom types
...
final class RawArgumentsFetchTests {
    ...
    func testFetchIntEnum() {
        let node = RawArgumentsTestNode()
        // Pass the raw value since Foundry doesn't know about our Swift enum
    ...
    // MARK: - FoundryBuiltinConvertible custom type tests
