...
protocol FoundryEncodingContainer {
...
class FoundryEncoder: Encoder {
    var codingPath: [any CodingKey] = []
    var userInfo: [CodingUserInfoKey : Any] = [:]
    var container: FoundryEncodingContainer? = nil
    ...
    func container<Key>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> where Key : CodingKey {
        ...
        let container = FoundryKeyedContainer<Key>(codingPath: codingPath, userInfo: userInfo)
...
//        _ = key
    ...
    func unkeyedContainer() -> any UnkeyedEncodingContainer {
        let container = FoundryUnkeyedContainer(codingPath: codingPath, userInfo: userInfo)
        self.container = container
        return container
    }

    func singleValueContainer() -> any SingleValueEncodingContainer {
        let container = FoundrySingleValueContainer(codingPath: codingPath, userInfo: userInfo)
        self.container = container
        return container
    }

    class FoundryKeyedContainer<Key:CodingKey>: KeyedEncodingContainerProtocol, FoundryEncodingContainer {
...
//            _ = container
        ...
        var codingPath: [any CodingKey] = []
        var userInfo: [CodingUserInfoKey: Any]
        var storage: [String:FoundryEncodingContainer] = [:]
        ...
        func nestedSingleValueContainer(forKey key: Key)
            -> FoundrySingleValueContainer
        {
            let container = FoundrySingleValueContainer(
    ...
    class FoundryUnkeyedContainer: UnkeyedEncodingContainer, FoundryEncodingContainer {
        var codingPath: [any CodingKey] = []
        var userInfo: [CodingUserInfoKey: Any]
        ...
        func encode<T>(_ value: T) throws where T: Encodable {
            ...
            if let v = value as? Array<Encodable> {
                ...
                for i in 0..<v.count {
                    let v = v [i]
                    let nested = FoundrySingleValueContainer(codingPath: codingPath, userInfo: userInfo)
                    try nested.encode(v)
    ...
    class FoundrySingleValueContainer: SingleValueEncodingContainer, FoundryEncodingContainer {
        func encode<T>(_ value: T) throws where T : Encodable {
...
final class MemoryLeakTests {
    ...
    public func test_513_leak2() {
        ...
        func oneIteration(bytes: PackedByteArray) {
            let image0 = FoundrySwift.Image()
            let variant = Variant(image0)
            let image: FoundrySwift.Image = variant.asObject()!
    ...
    public func test_544_leak() {
        ...
        // https://docs.foundryengine.org/en/stable/classes/class_string.html#class-string-method-left
    ...
    public func test_551_leak() {
        checkLeaks {
            for i in 0 ..< 200 {
                let str = Foundry.str(arg1: Variant(i))
    ...
    @FoundrySwiftTest
    public func test_foundry_string_description_leak() {
        var buffer: String = ""
        checkLeaks {
    ...
    @FoundrySwiftTest
    public func test_foundry_string_from_variant_leak() {
        var buffer: String = ""
