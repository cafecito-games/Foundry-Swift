...
class MacroFoundryTestCase: XCTestCase {
    class var macros: [String: Macro.Type] {
        [:]
    }
    
    /// Set `FOUNDRYSWIFT_REGENERATE_MACRO_TEST_RESOURCES=1` to regenerate expected outputs into `Tests/FoundrySwiftMacrosTests/Resources`.
    /// You can also set `FOUNDRYSWIFT_REGENERATE_MACRO_TEST_RESOURCES=/absolute/path` to write elsewhere.
    let regeneratedResourcesPath: String? = {
        let envKey = "FOUNDRYSWIFT_REGENERATE_MACRO_TEST_RESOURCES"
    ...
    /// If `MacroFoundryTestCase.regeneratedResourcesPath` is not `nil`, it will regenerate the expected output into that path instead
