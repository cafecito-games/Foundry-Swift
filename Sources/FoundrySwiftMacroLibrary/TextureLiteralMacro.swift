...
public struct Texture2DLiteralMacro: ExpressionMacro {
    enum ProviderDiagnostic: String, DiagnosticMessage {
        ...
        var diagnosticID: MessageID {
            MessageID(domain: "FoundrySwiftMacros", id: rawValue)
        }
    }
    ...
    public static func expansion(of node: some FreestandingMacroExpansionSyntax,
        ...
        return """
        {
            guard let texture: Texture2D = Foundry.load(path: \(argument)) else {
                Foundry.pushError("Texture could not be loaded.", \(raw: location.file), \(raw: location.line))
                preconditionFailure(
