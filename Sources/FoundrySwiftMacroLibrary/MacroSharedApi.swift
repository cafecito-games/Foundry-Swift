...
enum FoundryMacroError: Error, DiagnosticMessage {
    case foundryMacroNotOnClass
    ...
    var message: String {
        ...
        case .foundryMacroNotOnClass:
            "@Foundry attribute can only be applied to a class"
        case .exportMacroNotOnVariable:
        ...
        case .nameCollision(let name):
            "Same name `\(name)` for two different declarations. FoundryScript doesn't support it."
    ...
    var diagnosticID: MessageID {
        MessageID(domain: "FoundrySwiftMacros", id: message)
    }
}
...
enum MacroError: Error {
    ...
    var localizedDescription: String {
        ...
        case .propertyGetSet:
            return "Properties exported to Foundry must be readable and writable"
        }
    }
}

struct SignalName {
    let foundryName: String
