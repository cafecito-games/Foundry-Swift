...
public struct FoundryExport: PeerMacro {    
    public static func expansion(of node: SwiftSyntax.AttributeSyntax, providingPeersOf declaration: some SwiftSyntax.DeclSyntaxProtocol, in context: some SwiftSyntaxMacros.MacroExpansionContext) throws -> [SwiftSyntax.DeclSyntax] {
        guard let variableDecl = declaration.as(VariableDeclSyntax.self) else {
            let classError = Diagnostic(node: declaration.root, message: FoundryMacroError.exportMacroNotOnVariable)
        ...
        for binding in variableDecl.bindings {
            ...
            if needsSetter {
                declarations.append("""
                static func _mproxy_set_\(raw: identifier)(pInstance: UnsafeRawPointer?, arguments: borrowing FoundrySwift.Arguments) -> FoundrySwift.FastVariant? {
                    guard let object = _unwrap(self, pInstance: pInstance) else {
                        FoundrySwift.Foundry.printErr("Error calling setter for \(raw: identifier): failed to unwrap instance \\(String(describing: pInstance))")
                        return nil
                    }
                
                    FoundrySwift._invokeSetter(arguments, "\(raw: identifier)", object.\(raw: identifier)) {
            ...
            declarations.append("""
            static func _mproxy_get_\(raw: identifier)(pInstance: UnsafeRawPointer?, arguments: borrowing FoundrySwift.Arguments) -> FoundrySwift.FastVariant? {
                guard let object = _unwrap(self, pInstance: pInstance) else {
                    FoundrySwift.Foundry.printErr("Error calling getter for \(raw: identifier): failed to unwrap instance \\(String(describing: pInstance))")
                    return nil
                }
            
                return FoundrySwift._invokeGetter(object.\(raw: identifier))
            }                        
            """)
        }
