...
/// The @Foundry macro detects this attribute and generates the rpcConfig call.
public struct FoundryRpc: PeerMacro {
    public static func expansion(
        ...
        guard declaration.as(FunctionDeclSyntax.self) != nil else {
            let error = Diagnostic(node: declaration.root, message: FoundryMacroError.rpcMacroNotOnFunction)
            context.diagnose(error)
            return []
        }

        // This is a marker macro - no peer code is generated.
        // The @Foundry macro will detect @Rpc and generate rpcConfig registration.
        return []
    }
}
