...
class SwiftSprite: Sprite2D {
    ...
    // features of this class with the Foundry engine.
    ...
    // This callback style receives the arguments from Foundry as an array of Variants, and returns a Variant
    ...
    override func _process (delta: Double) {
        ...
        Foundry.print("dictionary2: \(String(describing: dict2)) \(dict2?["type"]?.description ?? "no value for type") \(dict2?["value"]?.description ?? "no value for value")")
        
        // part b
        if let result = dict2?.get(key: Variant("type"), default: Variant(-1)) {
            let value = String(result)
            Foundry.print("2 Found this value \(value ?? "no value found")")
        }
...
public func swift_entry_point(
    foundryGetProcAddr: OpaquePointer?,
    libraryPtr: OpaquePointer?,
    extensionPtr: OpaquePointer?) -> UInt8
{
    print ("ManualExtension: Starting up")
    guard let foundryGetProcAddr, let libraryPtr, let extensionPtr else {
        return 0
    }
    initializeSwiftModule(foundryGetProcAddr, libraryPtr, extensionPtr, initHook: setupScene, deInitHook: { x in })
    return 1
}
