...
/// Creates the definition for a Swift class to be surfaced to Foundry.
///
/// This macro creates the required constructors that the FoundrySwift framework requires (the `init`, and the
...
/// `_process` run in editor, making the class work like `@tool` annotated script in FoundryScript
///
@attached(member,
          names: named (_initializeClass), named(classInitializer), named (implementedOverrides))
public macro Foundry(_ behavior: ClassBehavior = .gameplay) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "FoundryMacro")

public enum ClassBehavior: Int, Sendable {
    case gameplay, tool
}

/// Exposes the function to the Foundry runtime
///
/// When this attribute is applied to a function, the function is exposed to the Foundry engine, and it
/// can be called by scripts in other languages.
///
/// The parameters and returns type of the function must be `_FoundryBridgeable`.
///
/// - Parameter autoSnakeCase: if `true` (default value is `false`), the function name will be automatically translated from `camelCase` to `snake_case` when exposed to Foundry
@attached(peer, names: prefixed(_mproxy_), prefixed(_pproxy_))
public macro Callable(autoSnakeCase: Bool = false) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "FoundryCallable")

/// Exposes a property or variable to the Foundry runtime
///
/// When this attribute is applied to a variable or a computer property in a class, the values can be surfaced to the
/// Foundry editor and can participate in Foundry's serialization process.
///
/// The attribute can only be applied to properties and variables that can be stored in a Variant.
///
/// - Parameter hint: this is of type ``PropertyHint`` and can be used to tell the Foundry editor the
...
public macro Export(_ hint: PropertyHint = .none, _ hintStr: String? = nil, usage: PropertyUsageFlags = .default) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "FoundryExport")
...
public macro exportGroup(_ name: String, prefix: String = "") = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "FoundryMacroExportGroup")
...
@freestanding(expression)
public macro exportSubgroup(_ name: String, prefix: String = "") = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "FoundryMacroExportSubgroup")

/// A macro used to write an entrypoint for a Foundry extension and register scene types.
///
/// For example, to initialize a Swift extension to Foundry with custom types:
/// ```swift
/// class MySprite: Sprite2D { ... }
/// class MyControl: Control { ... }
///
/// #initFoundryExtension(cdecl: "myextension_entry_point",
///                     types: [MySprite.self, MyControl.self],
///                     enums: [MySprite.BlendMode.self])
/// ```
///
/// - Parameter cdecl: The name of the entrypoint exposed to C.
/// - Parameter types: The node types that should be registered with Foundry.
/// - Parameter enums: a list of enumerations that use an Int64 backing store and are CaseIterable
/// - Parameter registerDocs: Set to true if you want to locate documents relative to the dynamic library and load them into Foundry
...
public macro initFoundryExtension(cdecl: String,
                                types: [Wrapped.Type] = [],
                                enums: [(any CaseIterable).Type] = [],
                                registerDocs: Bool = false,
                                hookMethod: ((ExtensionInitializationLevel, Bool)->())? = nil
) = #externalMacro(module: "FoundrySwiftMacroLibrary",
                                                                        type: "InitFoundryExtensionMacro")

/// Macro used to write an entrypoint for a Foundry extension and register all the supported scene types.
///
/// When Foundry initializes your extension it does so in stages and you get
/// a chance to register the types that you want to expose to the Foundry engine for
/// each stage (``FoundryExtension/InitializationLevel``).
///
/// For example, to initialize a Swift extension to Foundry with some custom types
...
/// - Parameter hookMethod: Hook that is invoked after each init/deinit stage
@freestanding(declaration, names: named(enterExtension))
public macro initFoundryExtension(
    cdecl: String,
    ...
    module: "FoundrySwiftMacroLibrary",
    type: "InitFoundryExtensionMacro")
...
/// ```
//@freestanding(expression)
//public macro texture2DLiteral(_ path: String) -> Texture2D = #externalMacro(module: "FoundrySwiftMacroLibrary",
//                                                                            type: "Texture2DLiteralMacro")

// MARK: - Attached Macros

/// A macro that enables an enumeration to be visible to the Foundry editor.
///
/// Use this macro with `ClassInfo.registerEnum` to register this enumeration's visibility in the Foundry editor.
///
/// ```swift
/// @PickerNameProvider
...
/// ```
///
/// - Important: The enumeration should have an `Int` backing to allow being represented as an integer value by Foundry.
@attached(extension, conformances: CaseIterable, Nameable, names: named(name))
//@attached(member, names: named(name))
public macro PickerNameProvider() = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "PickerNameProviderMacro")
...
public macro NativeHandleDiscarding() = #externalMacro(module: "FoundrySwiftMacroLibrary",
...
@attached(accessor)
public macro SceneTree(path: String? = nil) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "SceneTreeMacro")
...
/// The generated property will be computed, and therefore read-only.
@attached(accessor)
public macro Node(_ path: String? = nil) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "SceneTreeMacro")

/// Defines a Foundry signal on a class.
///
/// The `@Foundry` macro will register any #signal defined signals so that they can be used in the editor.
///
/// Usage:
/// ```swift
/// @Foundry class MyNode: Node2D {
///     #signal("game_started")
...
/// ```
///
/// - Parameter signalName: The name of the signal as registered to Foundry.
/// - Parameter arguments: If the signal has arguments, they should be defined here as a dictionary of argument name to type. For
/// example, ["name" : String.self] declares that the signal takes one argument of string type. The argument name is provided to the foundry
/// editor. Argument types are enforced on the `emit(signal:_argument)` method. Argument types must conform to FoundryVariant.
@freestanding(declaration, names: arbitrary)
@available(*, deprecated, message: "Use the @Signal macro instead.")
public macro signal(_ signalName: String, arguments: [String: Any.Type] = [:]) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "SignalMacro")
...
@attached(accessor)
public macro Signal(_ names: String...) = #externalMacro(module: "FoundrySwiftMacroLibrary", type: "SignalAttachmentMacro")
