...
/// Variant objects box various Foundry Objects, you create them with one of the
...
///
/// A Variant:
/// - Can store almost any Foundry engine datatype.
/// - Can perform operations between many variants. FoundryScript uses Variant as its atomic/native datatype.
/// - Can be hashed, so it can be compared to other variants.
/// - Can be used to convert safely between datatypes.
/// - Can be used to abstract calling methods and their arguments. Foundry exports all its functions through variants.
...
public final class Variant: Hashable, Equatable, CustomDebugStringConvertible, _FoundryBridgeable, _FoundryNullableBridgeable {
    ...
    /// Initialize with existing `ContentType` assuming this ``Variant`` owns it since now. Fails if `content` represents Foundry Nil.
    @_spi(FoundrySwiftRuntimePrivate) public init?(takingOver otherContent: ContentType) {
        if otherContent == Variant.zero { return nil }
    ...
    /// Compares two variants, does this by delegating the comparison to Foundry
    public static func == (lhs: Variant, rhs: Variant) -> Bool {
        var valid = FoundryExtensionBool (0)
        let ret = Variant (false)
        
        gi.variant_evaluate (FOUNDRY_EXTENSION_VARIANT_OP_EQUAL, &lhs.content, &rhs.content, &ret.content, &valid)
    ...
    public convenience init(_ from: some BinaryInteger) {
        self.init(payload: Int64(from), constructor: VariantFoundryInterface.variantFromInt)
    }
    ...
    public convenience init(_ from: some BinaryFloatingPoint) {
        self.init(payload: Double(from), constructor: VariantFoundryInterface.variantFromDouble)
    }
    ...
    public convenience init(_ from: Bool) {
        let payload: FoundryExtensionBool = from ? 1 : 0
        self.init(payload: payload, constructor: VariantFoundryInterface.variantFromBool)
    }
    ...
    public convenience init(_ from: String) {
        ...
        self.init(payload: stringContent, constructor: FoundryInterfaceForString.variantFromSelf)
        FoundryInterfaceForString.destructor(&stringContent)
    }
    ...
    ///
    /// Attempts to cast the Variant into a FoundrySwift.Object, if the variant contains a value of type `.object`, then
    ...
    public var description: String {
        var ret = FoundryExtensionStringPtr (bitPattern: 0xdeaddead)
        gi.variant_stringify (&content, &ret)
        
        let str = stringFromFoundryString(&ret)
        FoundryInterfaceForString.destructor(&ret)
        return str ?? ""
    }
    ...
    public func call(method: StringName, _ arguments: Variant?...) -> Result<Variant?, CallErrorType> {
        ...
        var err = FoundryExtensionCallError()
        ...
        if err.error != FOUNDRY_EXTENSION_CALL_OK {
    ...
    public static func construct(type: Variant.GType) -> Result<Variant?,CallErrorType> {
        var newContent: ContentType = Variant.zero
        var err = FoundryExtensionCallError()
        gi.variant_construct(FoundryExtensionVariantType(rawValue: FoundryExtensionVariantType.RawValue(type.rawValue)), &newContent, nil, 0, &err)
        if err.error != FOUNDRY_EXTENSION_CALL_OK {
            return .failure(toCallErrorType(err.error))
        }
        return .success(Variant(takingOver: newContent))
    ...
    public subscript (index: Int) -> Variant? {
        get {
            ...
            var valid: FoundryExtensionBool = 0
            var oob: FoundryExtensionBool = 0
    ...
    public static func typeName(_ type: GType) -> String {
        let res = GString()
        gi.variant_get_type_name (FoundryExtensionVariantType (FoundryExtensionVariantType.RawValue(type.rawValue)), &res.content)
    ...
    /// Internal API. Returns ``PropInfo`` for when any ``Variant`` or ``Variant?`` is used in API visible to Foundry
    @inline(__always)
    @inlinable
    public static func _propInfo(
        ...
        _propInfoDefault(
            propertyType: .nil, // Foundry treats .nil as Foundry Variant
            name: name,
            hint: hint,
            hintStr: hintStr,
    ...
    /// Internal API.
    /// According to:
    /// - https://github.com/foundryengine/foundry/issues/67544#issuecomment-1382229216
    /// - https://github.com/foundryengine/foundry/blob/b6e06038f8a373f7fb8d26e92d5f06887e459598/core/doc_data.cpp#L85
...
public extension Variant.GType {
    /// Internal API. Foundry type name of the type with this variant tag.
