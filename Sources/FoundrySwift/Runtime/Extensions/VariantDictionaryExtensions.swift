    ...
    public subscript(key: (some VariantConvertible)?) -> FastVariant? {
        get {
            ...
            case .some(let key):
                ...
                if FoundryInterfaceForDictionary.keyed_checker(&content, &keyContent) != 0 {
                    var result = VariantContent.zero
                    FoundryInterfaceForDictionary.keyed_getter(&content, &keyContent, &result)
        ...
        consuming set {
            ...
            case .some(let newValue):
                ...
                case .some(let key):
                    var keyContent = key.content
                    FoundryInterfaceForDictionary.keyed_setter(&content, &keyContent, &newValueContent)
                case .none:
                    var keyContent = VariantContent.zero
                    FoundryInterfaceForDictionary.keyed_setter(&content, &keyContent, &newValueContent)
                }
            ...
            withUnsafePointer(to: keyContent) { pArg0 in
                withUnsafePointer(to: UnsafeRawPointersN1(pArg0)) { pArgs in
                    pArgs.withMemoryRebound(to: UnsafeRawPointer?.self, capacity: 1) { pArgs in
                        FoundryInterfaceForDictionary.method_erase(&content, pArgs, &result, 1)
                    }
                }
            }
        case .none:
