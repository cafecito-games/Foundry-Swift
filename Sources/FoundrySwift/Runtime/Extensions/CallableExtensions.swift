...
public extension Callable {
    ...
    ) where repeat each Argument: _FoundryBridgeable, R: _FoundryBridgeable {
    ...
    ) where repeat each Argument: _FoundryBridgeable {
        self.init { arguments in
            proxyCallableToSwiftClosure(arguments: arguments, closure: callback)
        }
    }
...
private func proxyCallableToSwiftClosure<each Argument, R>(
...
) -> Variant? where repeat each Argument: _FoundryBridgeable, R: _FoundryBridgeable {
    var index = 0
    ...
    do {
        ...
        Foundry.printErr("`Callable` invocation error: \(error.description)")
        return nil
    }
}
...
) -> Variant? where repeat each Argument: _FoundryBridgeable {
    var index = 0
