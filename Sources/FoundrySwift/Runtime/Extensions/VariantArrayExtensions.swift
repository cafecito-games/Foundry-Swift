...
extension VariantArray: CustomDebugStringConvertible {
    ...
    var typing: ContainerTypingParameter {
        ...
        case .object:
            ...
            guard let metatype = typeOfClass(named: className) else {
                Foundry.printErr("Unknown class name: \(className).")
                return .builtin(.nil)
            }
    ...
    /// - Parameter type: `T` the type of the elements in the VariantArray, must conform to `_FoundryContainerTypingParameter`.
    public convenience init<T: _FoundryContainerTypingParameter>(_ type: T.Type = T.self) {
        self.init(
            base: VariantArray(),
    ...
    /// - Parameter type: `T` the type of the elements in the VariantArray, must conform to `_FoundryNullableBridgeable`.
    ///
    /// ### Note
    /// It's the same as `init(T?.self)`.
    public convenience init<T: _FoundryNullableBridgeable>(_ type: T.Type = T.self) {
        self.init(T?.self)
    }
