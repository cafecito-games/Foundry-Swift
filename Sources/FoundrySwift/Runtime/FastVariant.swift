...
/// 24-bytes payload of Foundry Variant
// @usableFromInline
...
/// This type is guaranteed to contain a non-nil Foundry Variant payload.
public struct FastVariant: ~Copyable {
    ...
    /// Initialize `FastVariant` with raw `VariantContent`, assuming ownership over it. Fails if `VariantContent` represents Foundry nil.
    /// Unlike `init(unsafeTakingOver: VariantContent)` this function can be safely executed in the context where `VariantContent` has
    /// external origin and can represent Foundry nil.
    @inline(__always)
    @usableFromInline
    ...
    /// 2. has its ownership passed to Foundry, for example in ``VariantArray.setFastVariant(_:at:)``
    @inline(__always)
    @inlinable
    ...
    /// Initialize by requested an owned copy of the `VariantContent`. Fails if `VariantContent` represents Foundry nil, or copied variant is a `nil` Variant for some reason.
    @inline(__always)
    @usableFromInline
    ...
    /// Delicate API.
    ///
    /// Temporarily borrows `VariantContent` owned by Foundry
    /// to provide convenient API for extracting something from it.
    /// Fails if `VariantContent` represents Foundry nil, or copied variant is a `nil` Variant for some reason.
    ///
    ...
    /// Initialize ``FastVariant`` with raw `VariantContent`, assuming ownership over it. Assumes `VariantContent` doesn't represent Foundry nil.
    @inline(__always)
    @usableFromInline
