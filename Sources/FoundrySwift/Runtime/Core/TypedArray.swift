...
/// Descriptor of Foundry `Array` or `Dictionary` typing.
@usableFromInline
...
///
/// #### ❌ `Type 'YourType' does not conform to protocol '_FoundryContainerTypingParameter'`
...
/// Foundry doesn't guarantee non-nullability of `YourType` elements.
///
/// #### ❌ `'TypedArray' requires that 'YourType' conform to '_FoundryNullableBridgeable'`
/// You used `TypedArray<YourType?>` or `TypedDictionary` with `YourType?` `Key` or `Value`.
/// You should use `TypedArray<YourType>`, `TypedDictionary<YourType, _>`, `TypedDictionary<_, YourType>` instead.
/// Foundry guarantees non-nullability of `SomeType` elements.
public protocol _FoundryContainerTypingParameter: _FoundryBridgeable {
    /// Internal API.
    ...
    associatedtype _NonOptionalType: _FoundryBridgeable
    
    /// Internal API.
    /// `class_name` for given type as Foundry requires it
...
/// This type represents typed Foundry array, such as `Array[int]` or `Array[Object]`.
///
...
///
/// The `Element` reflects semantics of how Foundry treats nullability.
...
/// let element = array[1] // It's `Int`, Foundry guarantees that Array of builtin type elements doesn't contain `nil` values.
/// array.append(nil) // Illegal.
///
/// let otherArray: TypedArray<Object?> = [nil, nil, Object()]
/// let otherElement = otherArray[2] // It's `Object?`
///
/// let oneMoreArray: TypedArray<Object> // Illegal, Foundry can't guarantee that `Array[Object]` contains only non-null `Object`-s, use `TypedArray<Object?>`
/// ```
///
/// # Compilation troubleshooting
///
...
/// Foundry doesn't guarantee non-nullability of `Array[YourType]` elements.
///
/// #### ❌ `'TypedArray' requires that 'YourType' conform to '_FoundryNullableBridgeable'`
/// You used `TypedArray<YourType?>`.
/// You should use `TypedArray<YourType>` instead.
/// Foundry guarantees non-nullability of `Array[YourType]` elements.
public struct TypedArray<Element: _FoundryContainerTypingParameter>: CustomDebugStringConvertible, RandomAccessCollection, _FoundryBridgeableBuiltin, ExpressibleByArrayLiteral, Hashable {
    ...
    /// If not, a new ``VariantArray`` to wrap will be created by following Foundry rules of array type narrowing:
    ...
    public init?(_ variant: Variant) {
        ...
        withUnsafeMutablePointer(to: &content) { pPayload in
            variant.constructType(into: pPayload, constructor: FoundryInterfaceForArray.selfFromVariant)
        }
    ...
    /// Internal API. Returns ``PropInfo`` for when any ``TypedArray`` is used in API visible to Foundry
    @inlinable
    @inline(__always)
    public static func _propInfo(
        ...
        if Element._variantType == .nil {
            // .nil means `Variant` in Foundry in this context.
            // Foundry will see `TypedArray<Variant?>` as `Array`.
            // While `Array[Variant]` is an allowed FoundryScript statement,
    ...
    /// Resizes the array to contain a different number of elements. If the array size is smaller, elements are cleared, if bigger, new elements are `null`. Returns ``FoundryError/ok`` on success, or one of the other ``FoundryError`` values if the operation failed.
    ///
    ...
    /// Inserts a new element at a given position in the array. The position must be valid, or at the end of the array (`pos == size()`). Returns ``FoundryError/ok`` on success, or one of the other ``FoundryError`` values if the operation failed.
    ///
    /// > Note: This method acts in-place and doesn't return a modified array.
    ///
    ...
    @inline(__always)
    @inlinable
    public func front() -> Element? where Element: _FoundryBridgeableBuiltin {
        array.front().to()
    }
    ...
    @inline(__always)
    @inlinable
    public func front() -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
        unwrapOrCrash(
            array.front().to()
        )
    }
    ...
    @inline(__always)
    @inlinable
    public func back() -> Element? where Element: _FoundryBridgeableBuiltin {
        array.back().to()
    }
    ...
    @inline(__always)
    @inlinable
    public func back() -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
        unwrapOrCrash(
            array.back()
        )
    }
    ...
    @inline(__always)
    @inlinable
    public func pickRandom() -> Element? where Element: _FoundryBridgeableBuiltin {
        array.pickRandom().to()
    }
    ...
    @inline(__always)
    @inlinable
    public func pickRandom() -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
        unwrapOrCrash(
            array.pickRandom()
        )
    }
    ...
    @inline(__always)
    @inlinable
    public func popBack() -> Element? where Element: _FoundryBridgeableBuiltin {
        array.popBack().to()
    }
    ...
    @inline(__always)
    @inlinable
    public func popBack() -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
    ...
    @inline(__always)
    @inlinable
    public func popFront() -> Element? where Element: _FoundryBridgeableBuiltin {
        array.popFront().to()
    }
    ...
    @inline(__always)
    @inlinable
    public func popFront() -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
        unwrapOrCrash(
            array.popFront()
        )
    }
    ...
    @inline(__always)
    @inlinable
    public func popAt(position: Int64) -> Element? where Element: _FoundryBridgeableBuiltin {
    ...
    @inline(__always)
    @inlinable
    public func popAt(position: Int64) -> Element where Element._NonOptionalType: _FoundryNullableBridgeable {
        unwrapOrCrash(
...
public extension Variant {
    ...
    convenience init<T>(_ from: TypedArray<T>) where T: _FoundryContainerTypingParameter {
...
public extension FastVariant {
    ...
    init<T>(_ from: TypedArray<T>) where T: _FoundryContainerTypingParameter {
        self.init(from.array)
    }
