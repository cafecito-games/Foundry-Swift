...
public enum ArgumentAccessError: Error, Sendable, CustomStringConvertible {
    ...
    case foundryCallingConventionError
    ...
    public var description: String {
        ...
        case .foundryCallingConventionError:
            return "The expected Foundry calling convention was not met for RawArguments"
        case .couldNotSurfaceObject:
            return "Foundry's object handle could not be surfaced to Swift"
        }
    }
}
...
public struct Arguments: ~Copyable {
    ...
    enum Contents {
        ...
        /// Foundry passed internally managed buffer, retrieve values lazily
        case unsafeFoundryArguments(UnsafeFoundryArguments)
    }
    ...
    public var count: Int {
        ...
        case .array(let array):
            return array.count
        case .unsafeFoundryArguments(let contents):
    ...
    init(pargs: UnsafePointer<UnsafeRawPointer?>?, argc: Int64) {
        if let pargs, argc > 0 {
            contents = .unsafeFoundryArguments(.init(pargs: pargs, count: Int(argc)))
        } else {
            contents = .array([])
        }
    }
    ...
    public subscript(_ index: Int) -> Variant? {
        get {
            ...
            case .array(let array):
                return array[index]
            case .unsafeFoundryArguments(let args):
                do {
    ...
    /// It can be handy in the contexts where crash during OOB is inconvenient (parsing arguments of a call from Foundry side, for example).    
    public func argument(ofType: Variant?.Type = Variant?.self, at index: Int) throws(ArgumentAccessError) -> Variant? {
        ...
        case .unsafeFoundryArguments(let unsafeFoundryArgs):
            return try unsafeFoundryArgs.copy(at: index)
        }
    }
    ...
    public func withBorrowedFastVariant<T>(
        ...
        case .array(let array):
            if index >= 0 && index < array.count {
                ...
                if let variant {
                    ...
                    /// Prevent `FastVariant` from destroying content owned by Foundry Variant
        ...
        case .unsafeFoundryArguments(let unsafeFoundryArguments):
            return try unsafeFoundryArguments.withBorrowedFastVariant(at: index, use: use)
        }
    }
    ...
    public func argument(ofType: Variant.Type = Variant.self, at index: Int) throws(ArgumentAccessError) -> Variant {
        ...
        case .unsafeFoundryArguments(let unsafeFoundryArgs):
            variantOrNil = try unsafeFoundryArgs.copy(Variant.self, at: index)
        }
    ...
    public func argument(ofType: FastVariant.Type = FastVariant.self, at index: Int) throws(ArgumentAccessError) -> FastVariant {
        ...
        case .unsafeFoundryArguments(let unsafeFoundryArgs):
            variantOrNil = try unsafeFoundryArgs.copy(FastVariant.self, at: index)
        }
    ...
    public func argument(ofType: FastVariant?.Type = FastVariant?.self, at index: Int) throws(ArgumentAccessError) -> FastVariant? {
        ...
        case .unsafeFoundryArguments(let unsafeFoundryArgs):
            return try unsafeFoundryArgs.copy(FastVariant.self, at: index)
        }
    }
...
/// This helper class is sued to access the arguments that Foundry passes to functions
/// that you have exposed with @Callable - and provides direct access to the values in
/// there - it is part of Foundry's fast path so the number of arguments and the types are
...
public struct RawArguments: @unchecked Sendable {
    ...
    public func fetchArgument<T: _FoundryBridgeableBuiltin>(at index: Int) throws(ArgumentAccessError) -> T {
    ...
    public func fetchArgument<T: Wrapped>(at: Int) throws(ArgumentAccessError) -> T {
        guard let value = args[at] else {
            ...
            throw .foundryCallingConventionError
        }
        let ptr = value.assumingMemoryBound(to: UnsafeMutableRawPointer.self).pointee
        if let lookup = lookupLiveObject(handleAddress: ptr) {
            if let value = lookup as? T {
                ...
                let className = MainActor.assumeIsolated { lookup.foundryClassName.description }
...
//
// For the variants here that end up getting deallocated by FoundrySwift on their
...
public struct RawReturnWriter {
    ...
    ) where Key: _FoundryContainerTypingParameter, Value: _FoundryContainerTypingParameter {
    ...
    ) where Element: _FoundryContainerTypingParameter {
        writeResult(target, value.array)
    }

    public static func writeResult<T>(_ target: UnsafeMutableRawPointer?, _ value: T) where T: FoundryBuiltinConvertible {
        writeResult(target, value.toFoundryBuiltin())
    }
    ...
    public static func writeResult<T>(_ target: UnsafeMutableRawPointer?, _ value: T?) where T: _FoundryBridgeableBuiltin {
        if let value {
...
/// Execute `body` and return the result of executing it taking temporary storage keeping Foundry managed `Variant`s stored in `pargs`.
@inline(__always)
@inlinable
...
/// Internal API. Protocol covering types that have nullable semantics on Foundry Side: Object-derived types and Variant.
/// For example when Foundry says `Array[ObjectOrObjectSubclass]`, it actually means Array of `ObjectOrObjectSubclass?`
///
...
/// - `Variant`, because we differentiate between `Variant` with something inside and  Foundry `Variant` containing `null` (which is simply Swift `nil`)
public protocol _FoundryNullableBridgeable: _FoundryBridgeable {
}
...
///
/// ### Note
/// `Variant` and `ObjectOrObjectSubclass` themselves are _not_ `_FoundryContainerTypingParameter`.
/// Foundry doesn't guarantee that they are not null in the places where`_FoundryContainerTypingParameter` is used, and neither do we.
extension Optional: _FoundryContainerTypingParameter where Wrapped: _FoundryNullableBridgeable {
    ...
    /// Internal API.
    /// `class_name` for given `Optional` type as Foundry requires it
    ...
    public static var _className: StringName {
        if Wrapped.self == Variant.self {
            ...
            // TODO: Make Foundry macro generate this in a static context for every class, same for code generator
            "\(Wrapped.self)"
        }
    }
}

// Allows static dispatch for processing `Variant?` `Object?` types during  parsing callback ``Arguments`` or using them as arguments for invoking Foundry functions.
extension Optional: _FoundryBridgeable, VariantConvertible where Wrapped: _FoundryNullableBridgeable {
