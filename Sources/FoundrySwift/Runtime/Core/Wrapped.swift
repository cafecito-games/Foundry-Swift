...
//
// This is the base class for all types that Foundry calls "Classes", as
...
//
// The generated Foundry classes can be subclassed by the user, and their
// state is preserved if the object is passed to Foundry, and later it is
// resurfaced to the Swift universe.
//
// We ensure that all Foundry objects that are surfaced to Swift retain their
// identity.  So we keep a table of every surfaced Foundry object into Swift.
//
...
func pd (_ str: String) {
    #if false
    print("anothr")
    print ("FoundrySwift: \(str)")
    #endif
}

/// Wraps a non-Sendable value for use across Swift 6 concurrency boundaries where safety is
/// guaranteed by the caller (e.g. Foundry C callbacks known to be called on the main thread).
...
enum InitOrigin {
    ...
    /// The object surfaced from Foundry and Swift is wrapping an existing native instance.
    ///
    ...
    /// Foundry API return slot.
    case foundry

    /// Constructed by Foundry (from FoundryScript) in `createFunc`
    ///
...
///
/// The base class for all class bindings in Foundry, you should not have
...
/// pointer to the Foundry native object and also implements the Identifiable
/// protocol using this pointer.
///
/// Wrapped manages the lifecycle of these objects, and in the event that an
/// object in the Foundry world has been disposed, the handle in the Wrapped
...
/// on the object.
///
/// Wrapped subclasses come in two forms: straight bindings to the Foundry
...
/// explicitly relinquished by both Foundry and any references you might hold to
...
/// the Foundry conventions and the methods are prefixed with an underscore).
///
/// # LifeCycle
///
/// Some special handling is done for different objects in the Foundry hierarchy.
/// The way we now handle the lifecycle of Foundry objects in Swift is the following:
...
/// to it, and we keep a weak reference to it if there are no references to it in Foundry,
...
/// subtyped objects keep their state on the Swift side throughout the Foundry object's
...
/// Foundry object will always be destroyed by the Swift wrapper eventually.
///
/// # Type Registration
///
...
///
/// If you do not call this method, many of the overloads that Foundry would
/// call you back on will not be invoked.
@MainActor
open class Wrapped {
    /// Points to the underlying object
    public internal(set) nonisolated(unsafe) var handle: FoundryNativeObjectPointer?
    ...
    /// is necessary to ensure that Foundry knows which methods have been overwritten, and which
    /// ones Foundry will provide a default behavior for.
    ///
    /// This is necessary because the Foundry overwrite method does not surface a "base" behavior
    /// that can be called into.  Instead Foundry relies on the "Is the method implemented or not"
    /// to make this determination.
    ///
    /// If you are not using the `@Foundry` macro, you should overwrite this function and return
    /// the StringNames for the functions you override, like in this example, where we indicate
    /// that we override the Foundry `_has_point` method:
    ///
    /// ```
    ...
    @inline(never)
    public static func attemptToUseObjectFreedByFoundry() {
        fatalError ("Wrapped.handle was nil, which indicates the object was cleared by Foundry")
    }
    @_spi(FoundrySwiftRuntimePrivate)
    public typealias FoundryVirtualDispatchCallback = FoundryExtensionClassCallVirtual

    @_spi(FoundrySwiftRuntimePrivate) nonisolated open class func getVirtualDispatcher(name: StringName) -> FoundryVirtualDispatchCallback? {
        pd ("SWARN: getVirtualDispatcher (\"\(name)\") reached Wrapped on class \(self)")
        return nil
    }

    // Override this to add some custom logic to the class initialization of @Foundry annotated classes.
    // This function will only be called if the class in question is annotated with @Foundry
    ...
    deinit {
        ...
        if let handle {
            ...
            if self is RefCounted {
                // Foundry may already have consumed the final native reference before
    ...
    #if FOUNDRYSWIFT_WITH_MULTI_PROCESS
    static var bindingCallback = FoundryExtensionInstanceBindingCallbacks(
        create_callback: bindingCreate,
        free_callback: bindingFree,
        reference_callback: bindingReference)
    #else
    static var userTypeBindingCallback = FoundryExtensionInstanceBindingCallbacks(
        create_callback: userTypeBindingCreate,
        free_callback: userTypeBindingFree,
        reference_callback: userTypeBindingReference)

    static var frameworkTypeBindingCallback = FoundryExtensionInstanceBindingCallbacks(
    ...
    /// Returns the Foundry's class name as a `StringName`, returns the empty string on error
    public var foundryClassName: StringName {
    ...
    /// This method is posted by Foundry, you can override this method and
    ...
    ///  Called whenever Foundry retrieves value of property. Allows to customize existing properties.
    ...
    public func hasScript (method: StringName) -> Bool {
        guard let objectHasScriptMethod = gi.object_has_script_method else {
            fatalError("Foundry interface method 'object_has_script_method' was not available at runtime.")
        }
    ...
    public func callScript (method: StringName, _ arguments: Variant?...) throws -> Variant? {
        ...
        var error = FoundryExtensionCallError()
        guard let objectCallScriptMethod = gi.object_call_script_method else {
            fatalError("Foundry interface method 'object_call_script_method' was not available at runtime.")
        }
    ...
    ///
    /// In Foundry, some objects can be freed manually, and in particular
    ...
    public func free() {
        if let object = self as? Object, object.isClass("Node") {
            print ("FoundrySwift: Cannot call free() on Nodes; queueFree() should be used instead.")
            return
        }
        guard !(self is RefCounted) else  {
            print ("FoundrySwift: Cannot call free() on RefCounted; release all references to it instead.")
            return
        }
        guard isValid else {
            print ("FoundrySwift: free() called on an invalid object.")
            return
        }
        guard extensionInterface.objectShouldDeinit(object: self) else {
            print ("FoundrySwift: free() called with an invalid instance.")
            return
        }
    ...
    nonisolated open class var foundryClassName: StringName {
        fatalError("Subclasses of Wrapped must override foundryClassName")
    }

    nonisolated open class var classInitializer: Void { () }

    /// Indicates during which engine initialization stage this class is registered. `.scene` is default. This value is taken into consideration when using `#initFoundryExtension(cdecl:types:)`  or `EntryPointGeneratorPlugin`.
    nonisolated open class var classInitializationLevel: ExtensionInitializationLevel { .scene }
}

extension Wrapped: Equatable {
    /// Equality is based on the underlying Foundry native object handle.
    /// Two distinct Swift wrapper instances wrapping the same Foundry object are considered equal.
...
extension Wrapped: Identifiable {
    /// Conformance to Identifiable by using the underlying Foundry native object pointer.
...
public extension _FoundryBridgeable where Self: Wrapped {
    /// Initialize a new object.
    @MainActor init() {
        guard let nativeHandle = gi.classdb_construct_object(&Self.foundryClassName.content) else {
    ...
    @MainActor init(nativeHandle: FoundryNativeObjectPointer) {
        self.init(InitContext(handle: nativeHandle, origin: .swift))
    }
}
...
@MainActor func bindSwiftObject(_ object: some Wrapped, _ context: InitContext) {
    let name = object.self.foundryClassName
    let thisTypeName = StringName (stringLiteral: String (describing: Swift.type(of: object)))
    let frameworkType = thisTypeName == name

    var callbacks: FoundryExtensionInstanceBindingCallbacks = Wrapped.bindingCallback
...
#else
@MainActor func bindSwiftObject(_ instance: some Wrapped, toFoundry handle: FoundryNativeObjectPointer) {
    let name = instance.self.foundryClassName
    let thisTypeName = StringName (stringLiteral: String (describing: Swift.type(of: instance)))
    let frameworkType = thisTypeName == name
    
    var callbacks: FoundryExtensionInstanceBindingCallbacks
    if frameworkType {
    ...
    if frameworkType {
        ...
        //pd ("Registering instance with Foundry")
...
@_spi(FoundrySwiftRuntimePrivate) public
nonisolated(unsafe) var duplicateClassNameDetected: (_ name: StringName, _ type: Object.Type) -> Void = { name, type in
    preconditionFailure(
                """
                Foundry already has a class named \(name), so I cannot register \(type) using that name. This is a fatal error because the only way I can tell whether Foundry is handing me a pointer to a class I'm responsible for is by checking the class name.
                """
    )
}
...
func register<T: Object>(type name: StringName, parent: StringName, type: T.Type) {
    ...
    // The classdb_get_class_tag function is documented to return “a pointer uniquely identifying the given built-in class”. As of Foundry 4.2.2, it also returns non-nil for types registered by extensions. If Foundry is changed in the future to return nil for extension types, this will simply stop detecting duplicate class names. It won't break valid code.
...
@_spi(FoundrySwiftRuntimePrivate) public final class WrappedReference {
...
/// Registers the user-type specified with the Foundry system, and allows it to
/// receive any of the calls from Foundry virtual methods (those that are prefixed
/// with an underscore)
...
where T: RawRepresentable & CaseIterable, T.RawValue == Int64 {
    ...
    if split.count != 3 {
        Foundry.print("Could not register enum \(fullname) it needs 3 components")
        return
    }
...
nonisolated(unsafe) var liveFrameworkObjects: [FoundryNativeObjectPointer: WrappedReference] = [:]
nonisolated(unsafe) var liveSubtypedObjects: [FoundryNativeObjectPointer: WrappedReference] = [:]

public func printFoundrySwiftStats() {
...
#if FOUNDRYSWIFT_WITH_MULTI_PROCESS
nonisolated(unsafe) var pendingReleaseHandles: [UInt8: [FoundryNativeObjectPointer]] = [:]
#else
nonisolated(unsafe) var pendingReleaseHandles: [FoundryNativeObjectPointer] = []
#endif

private func objectClassName(_ handle: FoundryNativeObjectPointer) -> String {
    var sc: StringName.ContentType = StringName.zero
    if gi.object_get_class_name(handle, extensionInterface.getLibrary(), &sc) != 0 {
...
private func currentRefCount(_ handle: FoundryNativeObjectPointer) -> Int32? {
...
@inline(__always)
private func isTrackedHandle(_ handle: FoundryNativeObjectPointer) -> Bool {
    tableLock.withLock {
...
@inline(__always)
private func removePendingReleaseHandle(_ handle: FoundryNativeObjectPointer) {
    freeLock.withLockVoid {
        #if FOUNDRYSWIFT_WITH_MULTI_PROCESS
...
/// this is called automatically by Foundry's main loop iteration, but it is expose for the sake
/// of the test suite that wants to release objects without waiting for Foundry to run the queue
public func releasePendingObjects() {
    var copy: [FoundryNativeObjectPointer] = []
    ...
    for handle in copy {
        // During shutdown, Foundry may destroy RefCounted objects before the deferred
        // queue is flushed; if so, our instance binding free callbacks will remove
        // the handle from the live tables. In that case, don't call back into Foundry
...
func lookupLiveObject (handleAddress: FoundryNativeObjectPointer) -> Wrapped? {
    tableLock.withLock {
...
func lookupFrameworkObject (handleAddress: FoundryNativeObjectPointer) -> Wrapped? {
    tableLock.withLock {
        return liveFrameworkObjects [handleAddress]?.value
    }
}

func existingSwiftObject(for nativeHandle: FoundryNativeObjectPointer) -> Wrapped? {
    tableLock.withLock {
...
///
/// Use this when reconciling Swift wrapper identity with Foundry native lifetime.
/// Do not use it to distinguish whether the wrapper was initialized from Swift,
/// from Foundry, or from a FoundryScript-created user type. That is handled by
/// ``InitOrigin``.
public enum ReturnedObjectOwnership: Sendable {
    ...
    /// Foundry returned an owned `Ref<T>` wrapper that FoundrySwift should consume.
    case refWrapper

    /// Foundry returned an object pointer directly from the API return slot.
    ///
...
// - refWrapper: Foundry returned an owned Ref<> wrapper that Swift must consume.
// - foundryApiReturn: Foundry returned the object through a standard API return slot, so
...
/// Get an existing Swift object which is bound to Foundry `nativeHandle` or initialize a new one and bind it
// @_spi(FoundrySwiftRuntimePrivate)
#if FOUNDRYSWIFT_WITH_MULTI_PROCESS
...
let FoundrySwift_has_instance_binding = "FoundrySwift_has_instance_binding"

@MainActor public func getOrInitSwiftObject<T: Object>(nativeHandle: FoundryNativeObjectPointer, ownership: ReturnedObjectOwnership) -> T? {
    var hasInstanceBinding = Foundation.Thread.current.threadDictionary.object(forKey: FoundrySwift_has_instance_binding) as? ReferenceArray<Bool>
    if hasInstanceBinding == nil {
        hasInstanceBinding = ReferenceArray<Bool>()
        Foundation.Thread.current.threadDictionary[FoundrySwift_has_instance_binding] = hasInstanceBinding
    }
...
func createSwiftObject(nativeHandle: FoundryNativeObjectPointer) -> Object? {
    ...
    if let type = typeOfClass(named: className) {
        let created = type.init(InitContext(handle: nativeHandle, origin: .foundry))
        return created
    }
...
@MainActor public func getOrInitSwiftObject<T: Object>(nativeHandle: FoundryNativeObjectPointer, ownership: ReturnedObjectOwnership) -> T? {
    ...
    if let userType = userTypes[className] {
        let created = userType.init(InitContext(handle: nativeHandle, origin: .foundry))
    ...
    if let ctor = lookupFoundryType(named: className) as? T.Type {
        let result = ctor.init(InitContext(handle: nativeHandle, origin: .foundry))
    ...
    // from either the user or the foundry constructors, so we just
    // wrap what we know we can do: the type we have.
    //
    // So if we had say a "MyCamera3D" but we could not find it as a Foundry
    ...
    let result = T(InitContext(handle: nativeHandle, origin: .foundry))
    handleReturnedObject(staticType: T.self, object: result, ownership: ownership, isExistingWrapper: false)
    return result
}
#endif
...
nonisolated func createFunc(
    ...
    let handleInt: Int? = MainActor.assumeIsolated { () -> Int? in
        ...
        guard let type  = typeAny as? Wrapped.Type else {
            print ("FoundrySwift.createFunc: The wrapped value did not contain a type: \(typeAny)")
            return nil
        }

        guard let handle = gi.classdb_construct_object(&type.foundryClassName.content) else {
        ...
        guard let wrapper = object.wrapper else {
            fatalError("FoundrySwift.createFunc: wrapper should have been created during binding")
        }
...
nonisolated func recreateFunc(_ userData: UnsafeMutableRawPointer?, foundryObjectHandle: UnsafeMutableRawPointer?) -> UnsafeMutableRawPointer? {
    let userDataInt = userData.map { Int(bitPattern: $0) }
    let foundryObjectHandleInt = foundryObjectHandle.map { Int(bitPattern: $0) }
    let resultInt: Int? = MainActor.assumeIsolated {
        ...
        guard let foundryObjectHandleInt else {
            return nil
        }
        let foundryObjectHandle = UnsafeMutableRawPointer(bitPattern: foundryObjectHandleInt)!

        let typeAny = Unmanaged<AnyObject>.fromOpaque(userData).takeUnretainedValue()
        guard let type  = typeAny as? Wrapped.Type else {
            print ("FoundrySwift.recreateFunc: The wrapped value did not contain a type: \(typeAny)")
            return nil
        }
        #if FOUNDRYSWIFT_WITH_MULTI_PROCESS
        ...
        return Int(bitPattern: foundryObjectHandle)
    }
...
nonisolated func freeFunc (_ userData: UnsafeMutableRawPointer?, _ objectHandle: UnsafeMutableRawPointer?) {
    ...
    MainActor.assumeIsolated {
        ...
        // Release the unmanaged reference that was retained in bindFoundryInstance()
...
nonisolated func validatePropertyFunc(ptr: UnsafeMutableRawPointer?, _info: UnsafeMutablePointer<FoundryExtensionPropertyInfo>?) -> UInt8 {
    let ptrInt = ptr.map { Int(bitPattern: $0) }
    ...
    return MainActor.assumeIsolated {
        ...
        guard let infoPtr = infoInt.flatMap({ UnsafeMutablePointer<FoundryExtensionPropertyInfo>(bitPattern: $0) }) else { return 0 }
        ...
        if instance._validateProperty(&pinfo) {
            ...
            infoPtr.pointee.type = FoundryExtensionVariantType(FoundryExtensionVariantType.RawValue (pinfo.propertyType.rawValue))
...
nonisolated func bindingReference(_ token: UnsafeMutableRawPointer?, _ binding: UnsafeMutableRawPointer?, _ reference: UInt8) -> UInt8 {
    ...
    return MainActor.assumeIsolated {
        ...
        if reference != 0 {
            // In addition to a reference by FoundrySwift, Foundry also retained a reference.
            if rc == 2 {
            ...
            // Only FoundrySwift holds a reference, so we make the Wrapped's deinit available.
            if rc == 1 {
                if let refCounted, refCounted.handle != nil {
                    ref.weakify()
                }
...
nonisolated func bindingCreate (_ token: UnsafeMutableRawPointer?, _ instance: UnsafeMutableRawPointer?) -> UnsafeMutableRawPointer? {
    ...
    let resultInt: Int? = MainActor.assumeIsolated {
        ...
        if let handle = object.handle {
            let frameworkType = String(describing: type(of: object)) == object.foundryClassName.description
            tableLock.withLockVoid {
                if frameworkType {
                    liveFrameworkObjects[handle] = reference
                } else {
        ...
        guard let hasInstanceBinding = Foundation.Thread.current.threadDictionary.object(forKey: FoundrySwift_has_instance_binding) as? ReferenceArray<Bool> else {
...
nonisolated func userTypeBindingCreate (_ token: UnsafeMutableRawPointer?, _ instance: UnsafeMutableRawPointer?) -> UnsafeMutableRawPointer? {
    // Foundry-cpp does nothing for user types
    ...
    MainActor.assumeIsolated {
        if let bindingInt {
            ...
            if let obj = reference.value {
                ...
                // invoke methods for objects that have been disposed by Foundry.
                if let handle {
                    removePendingReleaseHandle(handle)
                }
                obj.handle = nil
...
/// This function is called by Foundry to invoke our callable, and contains our context in `userData`,
...
nonisolated func invokeWrappedCallable(wrapperPtr: UnsafeMutableRawPointer?, pargs: UnsafePointer<UnsafeRawPointer?>?, argc: Int64, retPtr: UnsafeMutableRawPointer?, err: UnsafeMutablePointer<FoundryExtensionCallError>?) {
    ...
    MainActor.assumeIsolated {
        ...
        let err = errInt.flatMap { UnsafeMutablePointer<FoundryExtensionCallError>(bitPattern: $0) }
...
struct CallableWrapper {
    ...
    func invoke(arguments: borrowing Arguments, retPtr: UnsafeMutableRawPointer?, err: UnsafeMutablePointer<FoundryExtensionCallError>?) {
        if let methodRet = function(arguments) {
            gi.variant_new_copy(retPtr, &methodRet.content)            
        }
        err?.pointee.error = FOUNDRY_EXTENSION_CALL_OK
    }
...
///
public func getActiveHandles() -> [FoundryNativeObjectPointer] {
    var handles: [FoundryNativeObjectPointer] = []
...
public func clearHandles(_ handles: [FoundryNativeObjectPointer]) {
...
fileprivate func lookupFoundryType(named className: String) -> AnyClass? {
    // The format is:
    // MODULE: LENGHT + String
    // Type: LENGHT + String
    // C
    //
    // So "FoundrySwift.Node" becomes "12FoundrySwift4NodeC":
    //
    let candidates: [String] = [
        "12FoundrySwift\(className.count)\(className)C",
        "19FoundrySwiftRuntime\(className.count)\(className)C",
    ]
...
/// Find existing Foundry or User `Wrapped.Type` having a `className`
func typeOfClass(named className: String) -> Object.Type? {
    if let userType = userTypes[className] {
        return userType
    }
    if let type = lookupFoundryType(named: className) as? Object.Type {
        return type
    }
    return nil
}
