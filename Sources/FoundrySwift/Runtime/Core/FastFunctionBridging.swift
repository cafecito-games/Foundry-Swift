...
public func _registerSignal(_ signalName: StringName, in className: StringName, arguments: [PropInfo] = []) {
    withUnsafeTemporaryAllocation(of: FoundryExtensionPropertyInfo.self, capacity: arguments.count) { bufferPtr in
        ...
        withExtendedLifetime(arguments) {
            ...
            gi.classdb_register_extension_class_signal (extensionInterface.getLibrary(), &className.content, &signalName.content, ptr, FoundryExtensionInt(arguments.count))
...
public func _registerMethod(
    ...
    arguments: [PropInfo],
    function: @escaping BridgedFunction,
    ptrFunction: @escaping FoundryExtensionClassMethodPtrCall
) {
    let argPtr = UnsafeMutablePointer<FoundryExtensionPropertyInfo>.allocate(capacity: arguments.count)
    defer { argPtr.deallocate() }
    let argMeta = UnsafeMutablePointer<FoundryExtensionClassMethodArgumentMetadata>.allocate(capacity: arguments.count)
...
/// Internal API.
/// Unwraps an `Object` from a native Foundry object pointer.
@inline(__always)
...
private func call_func(
    ...
    if let returnValue, let ret {
        ...
        // Ownership over variant is transferred to Foundry
        ret.unsafelyForget()
    }
}
