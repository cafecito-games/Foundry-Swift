...
/// Provides support to expose Swift methods and signals to the Foundry runtime, making it callable
...
public class ClassInfo<T:Object> {
    ...
    /// Initializes a ClassInfo structure to register operations with Foundry
    ...
    public func registerSignal (name signalName: StringName, arguments: [PropInfo] = []) {
        withUnsafeTemporaryAllocation(of: FoundryExtensionPropertyInfo.self, capacity: arguments.count) { bufferPtr in
            guard let ptr = bufferPtr.baseAddress else {
                Foundry.print("Swift.withUnsafeTemporaryAllocation failed at `ClassInfo.registerSignal`")
                return
            }
            ...
            withExtendedLifetime(arguments) {
                ...
                gi.classdb_register_extension_class_signal (extensionInterface.getLibrary(), &name.content, &signalName.content, ptr, FoundryExtensionInt(arguments.count))
    ...
    /// Exposes a new method to the Foundry world with the specific name
    ///
    ...
    ///     // We are getting one integer if called from Foundry of type Int
    ...
    public func registerMethod (name: StringName, flags: MethodFlags, returnValue: PropInfo?, arguments: [PropInfo], function: @escaping (T) -> (borrowing Arguments) -> Variant?) {
        let argPtr = UnsafeMutablePointer<FoundryExtensionPropertyInfo>.allocate(capacity: arguments.count)
        defer { argPtr.deallocate() }
        let argMeta = UnsafeMutablePointer<FoundryExtensionClassMethodArgumentMetadata>.allocate(capacity: arguments.count)
        defer { argMeta.deallocate() }
        var i = 0
        for arg in arguments {
            argPtr [i] = arg.makeNativeStruct()
            argMeta [i] = FoundryExtensionClassMethodArgumentMetadata(FOUNDRY_EXTENSION_METHOD_ARGUMENT_METADATA_NONE.rawValue)
            i += 1
        }
        let returnMeta = FoundryExtensionClassMethodArgumentMetadata(FOUNDRY_EXTENSION_METHOD_ARGUMENT_METADATA_NONE.rawValue)
        var retInfo = FoundryExtensionPropertyInfo ()
        if let returnValue {
    ...
    /// with Foundry in `registerMethod`
    ///
    /// - Parameters:
    ...
    public func registerProperty (_ info: PropInfo, getter: StringName, setter: StringName) {
        var pinfo = FoundryExtensionPropertyInfo ()
...
///
/// The supported types are those that can be wrapped as a Foundry Variant type.
public struct PropInfo: @unchecked Sendable, CustomDebugStringConvertible {
    ...
    func makeNativeStruct () -> FoundryExtensionPropertyInfo {
            ...
            withUnsafeMutablePointer(to: &className.content) { classNamePtr in
                withUnsafeMutablePointer(to: &hintStr.content) { hintStrPtr in
                    FoundryExtensionPropertyInfo(
                        type: FoundryExtensionVariantType(FoundryExtensionVariantType.RawValue (propertyType.rawValue)),
...
nonisolated func bind_call (_ udata: UnsafeMutableRawPointer?,
                ...
                r_error: UnsafeMutablePointer<FoundryExtensionCallError>?){
    ...
    MainActor.assumeIsolated {
        ...
        if let returnValueInt, let ret {
            ...
            // See https://github.com/foundryengine/foundry/issues/67544#issuecomment-1382229216
            if finfo.value.retType != .nil && ret.gtype != finfo.value.retType {
                ...
                if let rErrorInt {
                    let rError = UnsafeMutablePointer<FoundryExtensionCallError>(bitPattern: rErrorInt)!
                    rError.pointee.error = FOUNDRY_EXTENSION_CALL_ERROR_INVALID_METHOD
                }
                return
            }
            ...
            // Since we are giving control to Foundry of this variant, we need to make sure that
