...
/// This type represents typed Foundry dictionary, such as `Dictionary[int, String]` or `Dictionary[String, Object]`.
///
...
///
/// Both `Key` and `Value` reflect semantics of how Foundry treats nullability.
/// Allowed types are:
...
/// Foundry doesn't guarantee non-nullability of `ObjectType` keys or values.
///
/// #### ❌ `'TypedDictionary' requires that 'YourType' conform to '_FoundryNullableBridgeable'`
/// You used `YourType?` as `Key` or `Value` generic parameter.
/// You should use `YourType` instead.
/// Foundry guarantees non-nullability of `SomeType` when used as `Key` or `Value`.
public struct TypedDictionary<Key: _FoundryContainerTypingParameter, Value: _FoundryContainerTypingParameter>: CustomDebugStringConvertible, _FoundryBridgeableBuiltin, Sequence {
    ...
    /// Subscript operator for where `Value` is Foundry builtin type.
    ...
    /// ```
    public subscript(key: Key) -> Value? where Value: _FoundryBridgeableBuiltin {
        get {
    ...
    public subscript(key: Key) -> Value where Value._NonOptionalType: _FoundryNullableBridgeable {
        get {
            let variant = dictionary[key]
    ...
    /// If not, a new ``VariantDictionary`` to wrap will be created by following Foundry rules of dictionary type narrowing:
    ...
    public init?(_ variant: Variant) {
        ...
        withUnsafeMutablePointer(to: &content) { pPayload in
            variant.constructType(into: pPayload, constructor: FoundryInterfaceForDictionary.selfFromVariant)
        }
    ...
    /// Internal API. Returns ``PropInfo`` for when any ``TypedDictionary`` is used in API visible to Foundry
    @inlinable
    @inline(__always)
    public static func _propInfo(
        ...
        if Key._variantType == .nil && Value._variantType == .nil {
            ...
            // Foundry will see `TypedDictionary<Variant?, Variant?>` as `Dictionary`.
            // While `Dictionary[Variant, Variant]` is an allowed FoundryScript statement,
    ...
    /// Internal API. Returns ``PropInfo`` for when any ``TypedDictionary`` is used as a return value in API visible to Foundry
    @inlinable
    @inline(__always)
    ...
    /// Internal API. Returns ``PropInfo`` for when any ``TypedDictionary`` is used as a function argument in API visible to Foundry
    @inline(__always)
    @inlinable
    ...
    ///
    /// In FoundryScript, this is equivalent to the `in` operator:
    ///
...
public extension Variant {
    ...
    convenience init<Key, Value>(_ from: TypedDictionary<Key, Value>) where Key: _FoundryContainerTypingParameter, Value: _FoundryContainerTypingParameter {
...
public extension FastVariant {
    ...
    init<Key, Value>(_ from: TypedDictionary<Key, Value>) where Key: _FoundryContainerTypingParameter, Value: _FoundryContainerTypingParameter {
        self.init(from.dictionary)
    }
