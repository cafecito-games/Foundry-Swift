...
Follow up on the fundamental building block of Foundry's data types.

## Overview

You will often find the type ``Variant`` in Foundry source code.   Variants are
Foundry's way of passing around certain data types.  They are similar to Swift's
`Any` type, but they can only hold Foundry types (most structures and classes
...
* Foundry's native types: ``GString``, ``Vector``, ``Rect``, ``Transform``, ``Plane``, ``Quaternion``,
  ``AABB``,  ``Basis``, ``Projection``, ``Int64``, ``NodePaths``, ``RIDs``, ``Callable``, ``VariantDictionary``, ``Array``
  and PackedArrays.  
* Swift types that FoundrySwift adds convenience conformances for: ``Bool``, ``Int`` (and all signed/unsigned width varieties such as `UInt32`, `Int16`), ``String``, ``Float`` and ``Double``,
* Foundry's objects: e.g. ``Node``, ``Area2D``
...
paris of file names and sizes into a Foundry ``VariantArray`` with
...
## Extracting Foundry-derived objects from Variants

You can use the same principles to turn a Foundry object encoded into a Variant
into a FoundrySwift object that you used above like `Node.fromVariant()` or you
...
that we need to ensure that for any given Foundry-object, only a single
FoundrySwift.Object exists.  
...
func foo(_ array: VariantArray) {
    let variant = array[4] // returns Variant? since Foundry arrays can store `nil`s
}
```

## Calling Variant Methods

It is possible to invoke the built-in variant methods that exist in the Foundry
...
FoundrySwift surfaces Variants in two ways, one is `Variant` that we have
...
FoundrySwift surfaces the Variant type in returns and as input parameters to its
API.   
...
counterpart in terms of performance.  This is used internally in FoundrySwift, and
