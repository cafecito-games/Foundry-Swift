# Running Code in the Foundry Editor

There are cases where you might want some of your Swift code for your extension
to run while it is being used in the Foundry Editor.

To achieve this, you can include the `.tool` parameter to the `@Foundry` macro,
like this:
...
> Warning: `.tool` extensions run inside the editor, and let you access the 
  ...
  potential misuse of `@Foundry(.tool)` scripts. Be extremely cautious when 
