...
lifetime of your game. They can be accessed from any code, including FoundryScript,
...
1. **Built-in Foundry Singletons**: These are accessed using static methods on their
...
A singleton is defined like any other custom type. Use the `@Foundry` macro and
...
```swift
func setupScene(level: FoundryExtension.InitializationLevel) {
    if level == .scene {
...
```swift
func teardownScene(level: FoundryExtension.InitializationLevel) {
    if level == .scene {
...
The `MainLoop` in Foundry is always a `SceneTree` and is always present. You can
...
- This is exactly how Foundry's "Autoload Singletons" work internally

### Safe Registration of Node Singletons

It is **not** safe to add a node to the scene tree during FoundryExtension
