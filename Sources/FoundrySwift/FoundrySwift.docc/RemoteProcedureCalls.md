...
Use the `@Rpc` macro to configure methods for Foundry's multiplayer RPC system.

## Overview

Remote Procedure Calls (RPC) allow you to call methods on other peers in a
multiplayer game. In Foundry, RPC methods need to be configured with settings
that control who can call them and how they are transmitted over the network.

FoundrySwift provides the `@Rpc` macro to declaratively configure RPC methods,
similar to FoundryScript's `@rpc` annotation. The configuration is automatically
...
The `@Callable` macro exposes the method to Foundry, and `@Rpc` configures it
...
When you use the `@Rpc` macro, the `@Foundry` macro automatically generates
...
Once configured, you can call RPC methods using Foundry's standard RPC API:
