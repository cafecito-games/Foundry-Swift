# Documenting your Foundry Extension

If you are developing a Foundry extension that is intended to be consumed from
Foundry you might want to provide documentation for it.

During the development process, Foundry has a mechanism to generate a skeleton XML
...
Foundry would update the XML files in place with changes - so the documentation
you have written is not wiped out on every run.

In the Foundry world, plugins surface documentation when they are loaded by the
Foundry editor as part of the initialization sequence.   
...
Foundry using one of the `EditorInterop.loadHelp` methods.

In MacOS the preferred distribution mechanism for libraries are called
"frameworks", and FoundrySwift has built-in support for registering the
documentation on your behalf.  In other platforms, you need to manually
configure this.

## Generating XML API Definitions

Once you have a working extension, you need to invoke the Foundry editor from
a directory that contains a Foundry project that uses your extension, and then
invoke the editor like this:

```
$ cd your-test-game
$ foundry.editor --headless --path . --doctool . --foundry_extension-docs
```
...
On MacOS, if you are using the '#initFoundryExtension' macro, merely set the
'registerDocs' parameter to 'true' and the runtime will load the documentation
from the framework bundle for you.

If you are not using the '#initFoundryExtension' macro, you need to call the
