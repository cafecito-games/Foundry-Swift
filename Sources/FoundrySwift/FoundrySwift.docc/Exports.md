...
In Foundry, class members can be exported. This means their value gets saved along
...
functions to the Foundry world, see the <doc:CustomTypes> document.
...
1.2. All builtin Foundry types present in ``FoundrySwift``, such as ``VariantArray``, ``Vector3``, ``Callable``, ``Signal``
...
The `@Export` macro only works in your `@Foundry` class declaration, and will not work on Swift class extensions.
...
It is possible to group your exported properties inside the Foundry Inspector with
...
Since Foundry 4.0, nodes can be directly exported without having to use NodePaths.
...
Exporting NodePaths like in Foundry 3.x is still possible, in case you need it:
...
To surface arrays in Foundry, use a strong type for it, for example:
...
Alternatively, if you want to surface an array of Foundry objects, or even your own subclasses of those, use `TypedArray<YourCustomType?>`, for example:
...
That's due to Foundry absence of guarantee that that `Array[Object]` doesn't contain `nil`s.
