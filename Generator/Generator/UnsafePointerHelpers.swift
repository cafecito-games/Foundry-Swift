...
/// Generate methods to help marshaling arguments to Foundry while keeping things civil and on stack
...
/// Generate a struct that serves as a stack storage for multiple pointers, so that a pointer to it can be passed to Foundry as `pargs` parameter.
///
/// ```swift
...
private func generateUnsafeRawPointersN(_ p: Printer, pointerCount count: Int) {
    p("@_spi(FoundrySwiftRuntimePrivate) public struct UnsafeRawPointersN\(count)") {
        for i in 0..<count {
    ...
    p("extension UnsafeRawPointersN\(count)") {
        ...
        p("@_spi(FoundrySwiftRuntimePrivate) public init(\(args))") {
            for i in 0..<count {
                p("self.p\(i) = p\(i)")
            }
        }
