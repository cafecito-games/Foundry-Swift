...
@MainActor func registerEnumDefinition(_ enumDef: JFoundryGlobalEnumElement, prefix: String?) {
    guard let prefix else { return }
    globalEnums[prefix + enumDef.name] = enumDef
}

@MainActor func registerEnumDefinitions(_ values: [JFoundryGlobalEnumElement], prefix: String?) {
    guard let prefix else { return }
...
@MainActor func findEnumDef (name: String) -> JFoundryGlobalEnumElement? {
...
@MainActor func generateEnums (_ p: Printer, cdef: JClassInfo?, values: [JFoundryGlobalEnumElement], prefix: String?) {
    for enumDef in values {
        ...
        if isBitField || enumDef.name == "ConnectFlags" {
            let optionTypeName = getFoundryType (SimpleType (type: enumDef.name))
        ...
        p ("public enum \(getFoundryType (SimpleType (type: enumDefName))): Int64, CaseIterable\(extraConformances)") {
            var used = Set<Int> ()
            
            func getName (_ enumVal: JFoundryValueElement) -> String? {
            ...
            if enumDefName == "Error" {
                ...
                p ("public var localizedDescription: String { Foundry.errorString(error: self.rawValue) }")
            }
        }
