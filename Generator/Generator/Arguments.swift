...
func foundryArgumentToSwift (_ name: String) -> String {
    return escapeSwift (snakeToCamel (name))
}

@MainActor func isSmallInt(_ arg: JFoundryArgument) -> Bool {
    if arg.type != "int" {
        return false
    }
    switch getFoundryType(arg, kind: .classes) {
...
@MainActor func getArgumentDeclaration(_ argument: JFoundryArgument, omitLabel: Bool, kind: ArgumentKind = .classes, isOptional: Bool) -> String {
    ...
    if var dv = argument.defaultValue, dv != "" {
        ...
        // Given a dv of "Vector (1,0)" returns a FoundrySwift suitable "Vector(x: 1, y: 0)" based on the
        ...
        if !argumentType.starts(with: "Array") && !argumentType.starts(with: "bitfield::") && (!isStruct(argumentType) || isPrimitiveType(name: argumentType)) && argumentType != "NodePath" && !argumentType.starts(with: "typedarray::") && !argumentType.starts (with: "Dictionary") && dv != "null" {
            if argument.type == "String" {
                ...
                // FoundryScript has StringName literals, e.g. &"example" == StringName("example")
            ...
            case let at where at.hasPrefix("typedarray::"):
                ...
                if dv == "[]" || dv.hasSuffix("]([])") && !dv.contains("RDPipelineSpecializationConstant"){
                    def = " = \(getFoundryType(argument, kind: kind)) ()"
                } else {
            ...
            case "Array":
                if dv == "[]" {
                    ...
                    // Tracked: https://github.com/migueldeicaza/FoundrySwift/issues/7
    ...
    return "\(prefix)\(foundryArgumentToSwift (argument.name)): \(optNeedInOut)\(getFoundryType(argument, kind: kind))\(isOptional ? "?" : "")\(def)"
}

@MainActor func getArgRef (arg: JFoundryArgument) -> String {
    var argref: String
    var optstorage: String
    var needAddress = "&"
    if !isStruct(arg.type) { // { ) isCoreType(name: arg.type){
        argref = foundryArgumentToSwift (arg.name)
