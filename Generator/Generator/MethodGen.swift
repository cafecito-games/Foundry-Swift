...
/// Parsed `JFoundryArgument` that derives what's the proper strategy for processing the argument and marshaling it
struct MethodArgument {
    enum Translation {
        /// e.g. Float, Vector3
        case direct
        
        /// e.g. Foundry Variant to Swift Variant?
        case variant
    ...
    @MainActor init(from src: JFoundryArgument, typeName: String, methodName: String, options: TranslationOptions) throws {
        ...
        self.name = foundryArgumentToSwift(src.name)
...
@MainActor func preparingMandatoryVariadicArguments(_ p: Printer, arguments: [JFoundryArgument], body: () -> Void) {
    ...
    func withNestedUnsafe(index: Int = 0) {
        if index >= arguments.count {
            ...
            let argument = arguments[index]
            let argumentName = foundryArgumentToSwift(argument.name)
...
@MainActor func generateMethodCall(_ p: Printer, isVariadic: Bool, arguments: [JFoundryArgument], methodArguments: [MethodArgument], call: (CallArgsRef, MarshaledArgumentsCount) -> String) {
    if !isVariadic {
...
@MainActor func generateMethod(_ p: Printer, method: MethodDefinition, className: String, cdef: JClassInfo?, usedMethods: Set<String>, generatedMethodKind: GeneratedMethodKind, asSingleton: Bool) throws -> String? {
    ...
    // Default method name
    var swiftMethodName: String = foundryMethodToSwift (method.name)
    ...
    if let methodHash = method.optionalHash {
        ...
        case .classMethod:
            p.staticProperty(visibility: staticVarVisibility, isStored: true, name: bindName, type: "FoundryExtensionMethodBindPtr") {
                p ("var methodName = FastStringName(\"\(method.name)\")")
            
                p ("return withUnsafePointer(to: &\(className).foundryClassName.content)", arg: " classPtr in") {
        ...
        case .utilityFunction:
            p.staticProperty(visibility: staticVarVisibility, isStored: true, name: bindName, type: "FoundryExtensionPtrUtilityFunction") {
                p ("var methodName = FastStringName(\"\(method.name)\")")
    ...
    if !method.isVirtual {
        ...
        if usedMethods.contains (method.name) {
            ...
            // Try to hide as much as possible, but we know that Foundry child nodes will want to use these
    ...
    let returnOptional = foundryReturnType == "Variant" || (foundryReturnTypeIsReferenceType && method.returnValue?.meta != .required)
    let returnType = getFoundryType(method.returnValue) + (returnOptional ? "?" : "")

    /// returns appropriate declaration of the return type, used by the helper function.
    let frameworkType = foundryReturnTypeIsReferenceType
    func returnTypeDecl() -> String {
        if returnType != "" {
            guard let foundryReturnType else {
                fatalError ("If the returnType is not empty, we should have a foundryReturnType")
            }
            if method.isVararg {
                if foundryReturnType == "String" {
                    return "let _result = GString()"
                } else {
                    return "var _result: Variant.ContentType = Variant.zero"
                }
            } else if foundryReturnType.starts(with: "typedarray::") {
                let (storage, initialize) = getBuiltinStorage ("Array", asComputedProperty: false)
                return "var _result: \(storage)\(initialize)"
            } else if foundryReturnType == "String" {
                return "let _result = GString ()"
            } else {
                if foundryReturnTypeIsReferenceType {
                    // frameworkType = true
                    return "var _result = FoundryNativeObjectPointer(bitPattern: 0)"
                } else {
                    if foundryReturnType == "Variant" {
                        return "var _result: Variant.ContentType = Variant.zero"
                    } else if foundryReturnType.starts(with: "enum::") {
                        return "var _result: Int64 = 0 // to avoid packed enums on the stack"
                    } else {
                        
                        var declType: String = "let"
                        if (argTypeNeedsCopy(foundryType: foundryReturnType)) {
                            if builtinFoundryTypeNames [foundryReturnType] != .isClass {
                                declType = "var"
                            }
                        }
                        if method.isVirtual && builtinFoundryTypeNames [foundryReturnType] != .isClass {
                            declType = "var"
                        }
                        return "\(declType) _result: \(returnType) = \(makeDefaultInit(foundryType: foundryReturnType))"
                    }
                }
            }
        }
    ...
    func getCallResultArgument() -> String {
        ...
        if returnType != "" {
            guard let foundryReturnType else { fatalError("foundryReturnType is nil!") }
            ...
            if method.isVararg {
            ...
            } else if argTypeNeedsCopy(foundryType: foundryReturnType) {
                let isClass = builtinFoundryTypeNames [foundryReturnType] == .isClass
                ...
                if foundryReturnType == "Variant" {
                    ...
                    ptrResult = "&_result"
                } else if builtinSizes [foundryReturnType] != nil {
                    ptrResult = "&_result.content"
                } else {
                    if method.isVirtual {
                        ptrResult = "&_result"
    ...
    func getReturnStatement() -> String {
        ...
        if method.isVararg {
            ...
            } else if returnType == "FoundryError" {
                return """
            ...
            return "guard let _result else { \(returnOptional ? "return nil" : "fatalError (\"Unexpected nil return from a method that should never return nil\")") } ; return getOrInitSwiftObject (nativeHandle: _result, ownership: .foundryApiReturn)\(returnOptional ? "" : "!")"
        } else if foundryReturnType?.starts(with: "typedarray::") ?? false {
            let defaultInit = makeDefaultInit(foundryType: foundryReturnType!, initCollection: "takingOver: _result")
            return "return \(defaultInit)"
        } else if foundryReturnType?.starts(with: "enum::") ?? false {
    ...
    p ("\(mainActorAttribute)\(declarationTokens)(\(argumentsList))\(returnClause)") {
        if staticAttribute == nil {
            p("if handle == nil { Wrapped.attemptToUseObjectFreedByFoundry() }")
        }
        if method.optionalHash == nil {
            if let foundryReturnType {
                p(makeDefaultReturn(foundryType: foundryReturnType))
            }
        } else {
            if returnType != "" {
                p(returnTypeDecl())
