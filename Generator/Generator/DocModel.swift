...
//    }
    ...
    func lookupConstant (_ txt: String.SubSequence) -> String {
        func lookInDef (def: JClassInfo, match: String, local: Bool) -> String? {
                ...
                for vp in ed.values {
                    if vp.name == match {
                        ...
                        if local && false{
                            ...
                            return getFoundryDocType(SimpleType (type: "enum::" + ed.name)) + "/" + escapeSwift (snakeToCamel (name))
                        }
                    }
                }
            }
            
            if let cdef = def as? JFoundryExtensionAPIClass {
            ...
            if let cdef = def as? JFoundryBuiltinClass {
                for x in cdef.constants ?? [] {
                    if match == x.name {
                        return "\(snakeToCamel (x.name))"
                    }
            ...
            for ev in ed.values {
                if ev.name == txt {
                    ...
                    if let dot = ed.name.lastIndex(of: ".") {
                        ...
                        type = getFoundryDocType (SimpleType (type: containerType)) + "."
                        lookup = String (ed.name [ed.name.index(dot, offsetBy: 1)...])
                    }
                    let name = dropMatchingPrefix(lookup, ev.name)
                    
                    return "``\(type)\(getFoundryDocType (SimpleType (type: lookup)))/\(escapeSwift(String (name)))``"
                }
            }
        }
        ...
        if type != "" {
            if let ldef = classMap [type] {
                if let r = lookInDef(def: ldef, match: name, local: false) {
                    return "``\(getFoundryDocType(SimpleType (type: type)) + "/" + r)``"
                }
    ...
    func assembleArgs (_ optMethod: JFoundryClassMethod?, _ arguments: [JFoundryArgument]?) -> String {
        var args = ""
        ...
        for arg in arguments ?? [] {
            if optMethod != nil && i == 0 && optMethod!.name.hasSuffix(arg.name) {
                ...
                args.append(foundryArgumentToSwift(arg.name))
            }
            args.append(":")
            i += 1
        }
    ...
    func convertMethod (_ txt: String.SubSequence) -> String {
        ...
        func findMethod (name: String, on: JFoundryExtensionAPIClass) -> JFoundryClassMethod? {
            on.methods?.first(where: { x in x.name == name })
        }
        func findMethod (name: String, on: JFoundryBuiltinClass) -> JFoundryBuiltinClassMethod? {
            on.methods?.first(where: { x in x.name == name })
        }
        ...
        if let type {
            ...
            return "\(type)/\(foundryMethodToSwift(member))(\(args))"
        } else {
            if let apiDef = cdef as? JFoundryExtensionAPIClass {
                if let method = findMethod(name: member, on: apiDef) {
                    args = assembleArgs (method, method.arguments)
                }
            } else if let builtinDef = cdef as? JFoundryBuiltinClass {
        ...
        return "\(foundryMethodToSwift(member))(\(args))"
    }

    func convertMember (_ txt: String.SubSequence) -> String {
        let (type, member) = typeSplit(txt: txt)
        if let type {
            return "\(type)/\(foundryMethodToSwift(member))"
        }
        return foundryPropertyToSwift(member)
    }
    ...
    for x in lines {
        ...
        if #available(macOS 13.0, iOS 16.0, *) {
            ...
            mod = mod.replacing(rxConstantParam, with: { x in
                switch x.output.1 {
                case "param":
                    let pname = foundryArgumentToSwift (String(x.output.2))
            ...
            mod = mod.replacing(rxEnumMethodMember, with: { x in
                ...
                case "enum":
                    ...
                    return "``\(getFoundryDocType(SimpleType(type: "enum::" + x.output.2)))``"
            ...
            mod = mod.replacing(rxUrl, with: { x in
                let word = x.output.1
                let resolved = word.replacing("$DOCS_URL", with: "https://docs.foundryengine.org/en/")
