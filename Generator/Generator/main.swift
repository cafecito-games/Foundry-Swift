...
var rootUrl: URL {
    let url = URL(fileURLWithPath: #file)  // FoundrySwift/Generator/Generator/main.swift
        .deletingLastPathComponent()  // FoundrySwift/Generator/Generator
        .deletingLastPathComponent()  // FoundrySwift/Generator
        .deletingLastPathComponent()  // FoundrySwift
    return url
}
...
/// Special case for Xogot to avoid caching foundry interface pointers
...
if positionalArgs.count < 1 {
    print(
        ...
        - path-to-extension-api is the full path to extension_api.json from Foundry
        - output-directory is where the files will be placed
        - doc-directory is the Foundry documentation resides (foundry/doc)
...
let jsonApi = try! JSONDecoder().decode(JFoundryExtensionAPI.self, from: jsonData)
...
var globalEnums: [String: JFoundryGlobalEnumElement] = [:]

// Maps from a the class name to its definition
var classMap: [String: JFoundryExtensionAPIClass] = [:]

// Tracks whether a Foundry type has subclasses, we want to use this
...
@MainActor func validateClassFilter(_ names: Set<String>, label: String) {
    let missing = names.subtracting(classMap.keys).sorted()
    guard missing.isEmpty else {
        fatalError("""
        FoundrySwift Generator error: missing reference type '\(missing[0])' in \(label).

        The class filter references one or more classes that are not present in extension_api.json: \(missing.joined(separator: ", "))
        """)
    }
}

if classFilterProvided {
    validateClassFilter(classesToGenerate, label: "class filter")
}

if availableClassFilterProvided {
    validateClassFilter(availableClassNames, label: "available class filter")
}
...
/// - returns: True if the type is represented in Swift as simple `struct` with fields (or as a built-in Swift type), not wrapping a handle (pointer) to a native Foundry object.
@MainActor func isStruct(_ type: String) -> Bool { structTypes.contains(type) }

var builtinMap: [String: JFoundryBuiltinClass] = [:]
...
var builtinMemberOffsets: [String: [JFoundryMember]] = [:]
...
@MainActor struct Generator {
    func run() async throws {
        ...
        generateVariantFoundryInterface(coreDefPrinter)
