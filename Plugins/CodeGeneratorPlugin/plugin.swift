...
/// Generates the FoundrySwift API from Foundry's exported JSON API.
@main struct SwiftCodeGeneratorPlugin: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) throws -> [Command] {
        ...
        guard let config = try generationConfig(for: target.name, api: api) else {
            return []
        }
        ...
        let supportsMultiProcess = (target as? SwiftSourceModuleTarget)?
            ...
            .contains("FOUNDRYSWIFT_WITH_MULTI_PROCESS") == true
        ...
        if useCombinedOutput {
            ...
            for letter in letters {
                outputFiles.append(generatedSourcesDir.appending(path: "FoundrySwift\(letter).swift"))
            }
        ...
        return [
            Command.buildCommand(
                displayName: "Generating FoundrySwift API for \(target.name)",
    ...
    private func generationConfig(for targetName: String, api: URL) throws -> GenerationConfig? {
        switch targetName {
        // FoundrySwift is a single self-contained module: it generates the
        // builtins and every class, with no dependency on another module.
        case "FoundrySwift":
            return GenerationConfig(
                classFiles: try classFilesFromExtensionApi(api),
        ...
        case "FoundrySwiftCore":
            return GenerationConfig(
                classFiles: core.uniqued(),
                builtinFiles: [],
                preamble: """
@_exported import FoundrySwiftRuntime
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftRuntime
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftRuntime
""",
                dependencyClassFiles: runtime
            )
        case "FoundrySwiftControls":
            fallthrough
        case "FoundrySwift2D":
            fallthrough
        case "FoundrySwift3D":
            fallthrough
        case "FoundrySwiftGLTF":
            fallthrough
        case "FoundrySwiftXR":
            fallthrough
        case "FoundrySwiftEditor":
            fallthrough
        case "FoundrySwiftVisualShaderNodes":
            ...
            case "FoundrySwiftControls":
                classFiles = controls
                preamble = """
@_exported import FoundrySwiftCore
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftCore
"""
...
"""
            ...
            case "FoundrySwiftXR":
                ...
                preamble = """
@_exported import FoundrySwiftCore
@_exported import FoundrySwiftControls
@_exported import FoundrySwift3D
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftCore
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftControls
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwift3D
"""
...
@_exported import FoundrySwiftControls
@_exported import FoundrySwift3D
@_exported import FoundrySwiftGLTF
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftCore
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftControls
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwift3D
@_spi(FoundrySwiftRuntimePrivate) import FoundrySwiftGLTF
"""
...
"""
    ...
    private func classFilesFromExtensionApi(_ api: URL) throws -> [String] {
        let data = try Data(contentsOf: api)
        guard
            let object = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            let classes = object["classes"] as? [[String: Any]]
        else {
            throw NSError(
                domain: "FoundrySwift.CodeGeneratorPlugin",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Unable to read classes from \(api.path)"]
            )
        }
        return classes
    ...
    private func shouldUseCombinedOutput() -> Bool {
        if let value = ProcessInfo.processInfo.environment["FOUNDRYSWIFT_COMBINED_OUTPUT"]?.lowercased() {
